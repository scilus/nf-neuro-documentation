...
- **Contribute new modules and subworkflows** to `nf-neuro`, go to the
  {pagelink("module", "/guides/modules")} or {pagelink("subworkflow", "/guides/subworkflows")} contribution section.

## Discovering `nf-neuro`

To get information on `nf-neuro` components, you'll first need to install `python` and `nf-core`. We provide
extensive guidelines to do it in {pagelink("this guide", "/guides/basic_environment")}.
...
<Callout variant="warning" title="Important">
  First, follow the {pagelink("devops guide", "/guides/setup_environment")} to setup your
...
{pagelink("setup your environment", "/guides/setup_environment")}, for which we have devised clever ways to streamline the procedure.
Then, depending on which kind of component you want to submit, you'll either need to follow the
{pagelink("module creation", "/guides/modules")} or the {pagelink("subworkflow creation", "/guides/subworkflows")}
