...
import cli_modules_list from '@/assets/images/cli/nfcore_modules_list.png';
import cli_modules_info from '@/assets/images/cli/nfcore_modules_info.png';
import cli_subworkflows_list from '@/assets/images/cli/nfcore_subworkflows_list.png';
import cli_subworkflows_info from '@/assets/images/cli/nfcore_subworkflows_info.png';
...
- **Create a new pipeline quickly**, using **modules** and **subworkflows** from `nf-neuro`, go to the
  {pagelink("prototyping", "/guides/prototyping")} section.
- **Create or publish a production pipeline** branded with `nf-neuro`, go to the
  {pagelink("porting prototypes", "/guides/production")} section.
- **Contribute new modules and subworkflows** to `nf-neuro`, go to the
  {pagelink("module", "/guides/create-your-module/template")} or {pagelink("subworkflow", "/guides/subworkflows")} contribution section.

## Discovering `nf-neuro`

To get information on `nf-neuro` components, you'll first need to install `python` and `nf-core`. We provide
extensive guidelines to do it in {pagelink("this guide", "/guides/environments/nf-core")}.
...
```
<Image src={cli_modules_list} alt="nf-core modules list remote"/>
...
```
<Image src={cli_subworkflows_list} alt="nf-core subworkflows list remote"/>
...
```
<Image src={cli_modules_info} alt="nf-core modules info <category/module>"/>
...
```
<Image src={cli_subworkflows_info} alt="nf-core subworkflows info <subworkflow>"/>
...
<Callout variant="warning" title="Important">
  First, follow the {pagelink("devops guide", "/guides/environments/devops")} to setup your
...
{pagelink("setup your environment", "/guides/environments/devops")}, for which we have devised clever ways to streamline the procedure.
Then, depending on which kind of component you want to submit, you'll either need to follow the
{pagelink("module creation", "/guides/create-your-module/template")} or the {pagelink("subworkflow creation", "/guides/subworkflows")}
guidelines. Components that don't abide to them won't be accepted and PR containing them will be closed automatically.
