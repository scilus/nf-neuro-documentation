...
switched to **linear algebra format at the end**. Only because it makes it more readlable. Note that
**nextflow code is complied, so focus on readability**. 
:::
...
| Position | Keyword |  Type   |       Value       |                                                            Description                                                                   |
|----------|---------|---------|-------------------|------------------------------------------------------------------------------------------------------------------------------------------| 
|  first   |  index  | Integer |        > 0        | Position of the transformation in the sequence, following **linear algebra ordering**<br />(transform `N` comes first, `0` comes last).  |
|  any     |  from   | String  |        any        | Identifier for the **source space**.                                                                                                     |
|  any     |   to    | String  |        any        | Identifier for the **target space**.                                                                                                     |
|  any     |  mode   | String  | image<br />points | Kind of transformation. Either for **image** (voxel grids) or **points** (tractogram, surface, streamline).                              |
|  last    |  desc   | String  | affine<br />warp  | Type of transformation. Use **affine** to encompass any linear transformation, and **warp** for deformation and displacement fields.     |