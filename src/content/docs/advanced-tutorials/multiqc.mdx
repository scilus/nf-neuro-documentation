...
1. Pipeline following the `nf-core` conventions. For the purpose of this walkthrough,
we will reused the completed pipeline from the tutorial [Your pipeline from A to Z](/nf-neuro/pipeline/tutorial_explore/).
    ...
    checkout the `complete_pipeline` branch.
    ```bash
    git checkout complete_pipeline
    ```
    :::

2. Access to the [`nf-core` tools commands](https://nf-co.re/docs/nf-core-tools/)
...
1. A **subject-specific report** allowing QC of key processing steps using
figures (e.g., registration, segmentation, etc.).

2. A **global report** for quantitative metrics across a population.
...
<Tabs>
<TabItem label="New container">
```groovy
    container "${ 'multiqc/multiqc:v1.27.1' }"
```
</TabItem>
<TabItem label="Old container">
```groovy
...
To do so, let's add a `tag "$meta.id"` at the top of the process definition:

<Tabs>
<TabItem label="Added tag">
```groovy
process MULTIQC {
    tag "$meta.id"
...
top of your `PREPROC_DIFF` `main.nf`, right below the `main:` section:
    ```groovy
    ch_multiqc_files = Channel.empty()
    ```
...
channel to our previously empty channel.
    <Tabs>
    <TabItem label="QC files collected">
    ```groovy
    // DTI-derived metrics
...
section of the `PREPROC_DIFF` subworkflow
    ...
    <TabItem label="Original outputs">
    ```groovy
    emit:
        dwi                 = ch_dwi_bvalbvec.dwi           // channel: [ val(meta), dwi-raw ]
        dwi_denoised        = DENOISING_MPPCA.out.image     // channel: [ val(meta), dwi-after-mppca ]
...
`false` with `true`. We will also enable all the other metrics to create a more meaningful
QC report.
    <Tabs>
    <TabItem label="QC enabled">
    ```groovy
...
in the main workflow. We will use the same approach as step 3 above:
...
file, we can simply call it, and fill the [MultiQC inputs](#multiqc-inputs) with
...
3. <RunIcon class="inline-icon text-blue-300" aria-hidden /> **Now, let's test this out!**
...
### Customizing your MultiQC report
...
available in the standard Nextflow library. Let's fetch the config file
and supply it to the MultiQc module:
    ...
    <TabItem label="Without config file">
    ```groovy
    // MultiQC
    MULTIQC(
        ch_multiqc_files,
...
7. <RunIcon class="inline-icon text-blue-300" aria-hidden /> **Now, let's test this out!**
