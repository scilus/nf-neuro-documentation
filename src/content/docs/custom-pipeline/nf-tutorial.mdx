...
The nf-neuro tutorial is aimed at anyone wishing to use nf-neuro 
...
## Get nf-neuro-tutorial

The material to follow the tutorial is available here: https://github.com/scilus/nf-neuro-tutorial. 

The first step in this tutorial is to fork and/or clone or download the repository.
...
The nf-neuro-tutorial directory is a pre-configured repository containing the following elements at the root:
...
To keep things simple, we'll consider you want to process a dataset that 
contains for one subject a DWI and a T1 image, as follows :
...
This tutorial is divided into 11 sequential steps. 
Each step is used to complete the pre-configured main.nf file to obtain a complete 
workflow in the final steps. 
You can simply follow the steps from the main branch. 
However, if you encounter a problem in one of the steps, each step has a 
specific branch that you can jump straight to to continue the tutorial. 
This will also give you a point of comparison and help you understand the errors you encounter.


## The nf-neuro-tutorial folder
...
workflow get_data {
    main:
        ...
        t1_channel = Channel.fromFilePairs("$input/**/**/anat/*t1.nii.gz", size: 1, flat: true)
            { it.parent.parent.parent.name + "_" + it.parent.parent.name }
...
the main.nf file is pre-filled with a function to fetch data from the data folder
and a worflow to visualize the data. 

Run the worflow using :
```
...
### Step 3: Bind DTI module

### Step 4: Configure DTI module 

### Step 5: Add a new nf-neuro module
...
### Step 7: Add input for subworkflow

### Step 9: Configure subworkflow

### Step 10: Use output module in worflow

### Step 11: Create a local custom module
