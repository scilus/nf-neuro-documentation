...
with the name of the one you need and you'll be able to access the archives within your test suite :
...
without accessing the data itself. The only file required to be **snapshotted** is the **versions** file that keeps track
of dependencies :
...
2. The first assertion checks that the workflow ran successfully, using `workflow.success`.
3. The second assertion snapshots the **versions** file content and checks file naming, ensuring their consistency
...
:::tip[What is happening here?]
<Steps>
1. Channels names in the **list** should be empty and their size be `0`.
