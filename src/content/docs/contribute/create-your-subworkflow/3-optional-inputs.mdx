...
`ch_brain_mask` input in the `take` section of the subworkflow :
...
since the mask is **optional**. To associate it with another channel, use `join` with the special argument `remainder: true`,
...
<Steps>
1. `ch_anatomical` and `ch_brain_mask` are joined together, using `remainder: true`. Element in `ch_anatomical` are
   associated with ones in `ch_brain_mask`, if their `meta` field is identical (`[meta, image, mask]`). If no
   association is found in `ch_brain_mask`, the element gets appended with the value `null` ( `[meta, image, null]`).
2. Using the `map` operator, each element is verified. If the mask value is `null`, it is replaced with the
...
not as simple as doing a conditional test on `ch_brain_mask`, it will return `true` even if empty. Instead, you need to
**filter** the content of a component's input you wish to skip, **to empty it of all elements for which a mask exists** :
...
:::tip[What is happening here ?]
<Steps>
1. As previously, we join using `remainder: true`. Using `filter`, we remove all elements which are associated to a
