...
`ch_brain_mask` input in the <code class="text-blue-300">take</code> section of the subworkflow :
...
1. `ch_anatomical` and `ch_brain_mask` are joined together, using <InlineCode parts={frontmatter.coloredRemainder} />.
   ...
   If no association is found in `ch_brain_mask`, the element gets appended with the value <code class="text-blue-300">null</code>
   ( `[meta, image, null]`).
2. Using the `map` operator, each element is verified. If the mask value is <code class="text-blue-300">null</code>, it is replaced with the
...
not as simple as doing a conditional test on `ch_brain_mask`, it will return <code class="text-blue-300">true</code> even if
