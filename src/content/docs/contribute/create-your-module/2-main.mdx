...
- Environment variables available at runtime (`PATH` and others). **Use the prefix <code class="text-blue-400">\\$</code>** (e.g. `\$PATH`)
- Variables defined in the **groovy header**, in the `input` and `output` sections and through the `task` object. **Use
the prefix <code class="text-blue-400">$</code>** (e.g. `$prefix` or `$task.cpus`)
...
1. In the **groovy header**, two variables are defined for you : the `prefix` and the `args` list. They both use the
   [elvis operator](https://groovy-lang.org/operators.html#_elvis_operator) <code class="text-emerald-400">?:</code>, a
   ...
   or a default value.
   - The default value for the `prefix` is defined throught the <code class="text-orange-300">meta</code> variable, which is an
...
2. The scilpy command can handle its own parallelization. Using <code class="text-orange-300">export</code>,
   we declared **3 environment variables** to limit the threads used by dependencies (e.g. `ITK`, `OMP`, `OPENBLAS`).
3. The <code class="text-orange-300">scil_denoising_nlmeans.py</code> command is called on an input
   <code class="text-lime-300">$image</code>, a variable you will define in the `input` section later.
4. The denoised image is written to a file named <InlineCode parts={frontmatter.coloredDenoised} />, with
   ...
   - The brackets usage around <InlineCode parts={frontmatter.coloredPrefix} /> is equivalent to <code class="text-lime-300">$prefix</code>.
5. The user's custom arguments are passed to the command, by unraveling the <code class="text-orange-300">args</code> variable
   using <code class="text-orange-300">join</code>, a **groovy** operator.
...
:::note
Each channel defined below <code class="text-blue-300">input:</code> is independent. Their elements **have no association**
...
we need **two variables in input** : the <code class="text-orange-300">meta</code> and the <code class="text-lime-300">image</code>.
...
2. We defined a new variable `input_mask` in the **groovy header**. This variable will be used to store the value of the
   ...
   list `[]` is equivalent to <code class="text-blue-300">false</code>, combined with the [ternary operator](https://groovy-lang.org/operators.html#_ternary_operator).
...
<InlineCode parts={frontmatter.coloredDenoised} />. The matching glob pattern is <span class="text-orange-300">"*__denoised.nii.gz"</span>.
...
<code class="text-slate-200">task.ext</code> argument namespace. This will **enable users to
