...
All modules in **nf-neuro** contain a `stub` section, which requires a test case in the `tests/main.nf.test` file. It
...
For stubs, the only assertion needed is on the `version.yml` file that gets generated by the module. Here is a complete
stub test with assertion on the `version.yml` file :
