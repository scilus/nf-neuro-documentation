---
title: "Step 4: Install and use a new nf-neuro module"
...
[**Step 3: Use and configure a nf-neuro module**](/nf-neuro/pipeline/tutorial_step_3).
...
3. Understand input data specific to the module
    ...
    Similar to the DTI module, the MPPCA denoising module requires only the DWI image and an optional mask.
...
4. Configure the denosing module in the `nextflow.config` using the [API Documentation](/nf-neuro/api/modules/denoising/mppca) as a reference.
    
    Check the [solution](/nf-neuro/pipeline/tutorial_step_4/#4-verify-your-files) for the nextflow.config if needed.

5. Adapt the input for `RECONST_DTIMETRICS` module to attach it after the newly included denoised DWI module, reusing its outputs as inputs.
    ...
    <li>`.map{ it + [[]] }`:  Adds an empty list `([])` to each input tuple.</li>
        ...
        for the optional mask input. The `map` operation is similar to the one we did for the DTI module in [**Step 2**](/nf-neuro/pipeline/tutorial_step_2).
    </html>
    :::
</Steps>
