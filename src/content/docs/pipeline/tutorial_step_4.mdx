...
2. Use the module in the workflow. The MPPCA denoising module requires only the DWI image and an optional mask,
   opposed to the DTI_METRICS modules which takes the dwi volume with its bval/bvec directly.
